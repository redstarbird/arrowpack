/* 
    ...
    This file is WIP
    Will eventually be used for parsing JSON to rules for loading files and then more general lightweight JSON parser
...
struct FileRule * ParseJSONToStructArray(char * RawJSON) { // gets array of file rules from json
    ...
    while (1) {
        if (json[index] == '\0') {break;} // stop at end of file
        else if (json[index] == '[') {
            squareBracketDepth++;
