...
{
    const ScriptInjectData = "<script type=\"module\">" + fs.readFileSync(path.join(__dirname, "ClientDevServer.js")) +
                             "</script>"  // Websocket reloading script for the client

    const server = http.createServer(  // Creates the HTTP server to serve the webpages
        (req, res) => {
            const parsedURL = url.parse(req.url);
            var filePath = path.join(cwd, Settings.getValue("exit"),
                                     parsedURL.pathname)  // Find the path to the file`
            if (parsedURL.query)
            {
                filePath += path.sep;
                ...
                fs.readFile(filePath, (err, data) => {
                    if (err) {
                        ...
                        if (contentType === "text/html") {
                            ...
                            data = data.slice(0, InsertLocation) + ScriptInjectData +
                                   data.slice(InsertLocation);  // Insert the websocket reloading script
                        }
    ...
    wss.on("connection", (ws) => {
        ...
        ws.once("message", (message) => {
            message = path.join(Settings.getValue("entry"),
                                message.toString());  // Get file path client is on
            ...
            if (filePath[i] === value) {  // Check if the connection is to the given path
                if (!OpenedFiles.has(filePath[i])) {
                    ...
                    var FileContent = fs.readFileSync(
                        Settings.getValue("exit") + filePath[i].slice(Settings.getValue("entry").length), "utf8");
...
module.exports = {
    ...
    SendUpdatedPage: SendUpdatedPage
};