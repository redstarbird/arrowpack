...
{
    struct RegexMatch *SingleLineCommentLocations =
        GetAllRegexMatches(Text, "//[^\n]*\n", 0, 0);  // Finds locations of single-line comments
    int SingleLineCommentLocatsCount =
        RegexMatchArrayLength(SingleLineCommentLocations);       // Finds the number of single-line comments
