...
struct RegexMatch EMSCRIPTEN_KEEPALIVE *BasicRegexDependencies(char *filename, const char *pattern,
                                                               unsigned int Startpos, unsigned int Endpos,
                                                               struct RegexMatch *CommentLocations)
{
    ...
    {
        if (StringContainsSubstring(IteratePointer->Text, "https://") ||
            StringContainsSubstring(IteratePointer->Text,
                                    "http://"))  // Remove any matches that are URLs
        {
            ...
            if (CommentLocations != NULL)  // Checks if comments are present in the file
        {
            ...
            {
                struct RegexMatch *CurrentComment = &CommentLocations[0];  // Initialise variable to iterate
                                                                           // through comment locations

                while (CurrentComment->IsArrayEnd != true)  // Loops through comments
                {
                    if (IteratePointer->StartIndex > CurrentComment->StartIndex &&
                        IteratePointer->StartIndex <
                            CurrentComment->EndIndex)  // Checks if current dependency is inside of a comment
                    {
...
struct RegexMatch EMSCRIPTEN_KEEPALIVE *FindHTMLDependencies(struct Node *vertex, struct Graph **DependencyGraph)
{
    ...
    for (int i = 0; i < strlen(FileContents); i++)  // Search for the locations of comments in the file
    {
        ...
        if (FileContents[i] == '<' && FileContents[i + 1] == '!' && FileContents[i + 2] == '-' &&
            FileContents[i + 3] == '-')  // Search for start of comment
        {
            start = i;
            i = i + 3;
            while (i < strlen(FileContents) - 2)  // Search for end of comment
            {
                if (FileContents[i] == '-' && FileContents[i + 1] == '-' &&
                    FileContents[i + 2] == '>')  // End of comment has been found
                {
                    end = i + 2;
                    CommentLocationsFound++;
                    CommentLocations =
                        realloc(CommentLocations, sizeof(struct RegexMatch) * (CommentLocationsFound + 1));
    ...
    struct RegexMatch *HTMLIncludeMatches =
        BasicRegexDependencies(filename, "<include[^>]*src[^>]*=[^>]*\"[^>]*\"[^>]*", 0, 1,
                               CommentLocations);  // Find all HTML include matches
                        ...
                        for (int j = i + 3; j < strlen(IteratePointer->Text); j++) {
                            ...
                            if (equalsFound) {
                                if (IteratePointer->Text[j] == '"' || IteratePointer->Text[j] == '\'') {
                                    if (openStringFound) {
                                        if ((stringStartsDoubleParenthesis && IteratePointer->Text[j] == '"') ||
                                            (!stringStartsDoubleParenthesis && IteratePointer->Text[j] == '\'')) {
                                            /*dataTags[dataTagsFound] = malloc(sizeof(char) * (j -
                                            openStringLocation)); strncpy(IteratePointer->Text +
                                            openStringLocation, dataTags[dataTagsFound], j -
                                            openStringLocation - 1); dataTags[dataTagsFound][j -
                                            openStringLocation] = '\0'; dataTagsFound++; dataTags =
                                            realloc(dataTags, sizeof(char *) *
                                            (dataTagsFound + 1)); printf("Found src %s\n",
    ...
    MakeMatchesFullPath(HTMLIncludeMatches,
                        filename);  // Turn all HTML dependency paths into full paths
    ...
    /*
        ...
        ┃╰━╯┃┃╰━╯┃┃╰━╯┃
        ╰━━━╯╰━━━╯╰━━━╯*/
    // struct RegexMatch *CSSDependencies = BasicRegexDependencies(filename,
    // "<link\\s+rel\\s*=\\s*(\"')?stylesheet(\"')?\\s*(type\\s*=\\s*(\"')?text/css(\"')?)?\\s+href\\s*=\\s*(\"')?(.*?)(\"')?\\s*\\/>",
    // 5, 0);
    struct RegexMatch *CSSDependencies = BasicRegexDependencies(filename, "<link[^>$]*stylesheet[^>$]*", 0, 0,
                                                                CommentLocations);  // Find all CSS dependencies
        ...
        while (IteratePointer->IsArrayEnd == false)  // Gets link from CSS element
        {
            ...
            if (HREFLocation != -1)  // href location has been found
            {
                ...
                for (unsigned int i = HREFLocation; i < TextLength; i++) {
                    ...
                    {
                        if (TempStringPointer[i] != ' ' && TempStringPointer[i] != '\"' &&
                            TempStringPointer[i] != '\'')  // Search start of path
                        {
                            StartLocation = i;
                            break;
                        }
                ...
                if (StartLocation != -1)  // Start location has been found - search for end location
                {
                    for (unsigned int i = StartLocation; i <= TextLength; i++) {
                        if (TempStringPointer[i] == ' ' || TempStringPointer[i] == '\'' ||
                            TempStringPointer[i] == '\"' || TempStringPointer[i] == '>' ||
                            TempStringPointer[i] == '\0') {
            ...
            IteratePointer->Text =
                TurnToFullRelativePath(getSubstring(IteratePointer->Text, StartLocation, EndLocation - 1),
                                       GetBasePath(filename));  // Add path to matches array
            IteratePointer++;
        }
    }

    struct RegexMatch *JSDependencies = BasicRegexDependencies(filename, "<script[^>$]*", 7, 0,
                                                               CommentLocations);  // Search for Javascript dependencies
        ...
        while (IteratePointer->IsArrayEnd != true)  // Loop through each dependency
        {
            ...
            if (srcLocation != -1)  // src attribute has been found
            {
                ...
                if (StartLocation != -1)  // Start location has been found, search for end location
                {
                    for (unsigned int i = StartLocation; i <= TextLength; i++) {
                        if (IteratePointer->Text[i] == ' ' || IteratePointer->Text[i] == '\'' ||
                            IteratePointer->Text[i] == '\"' || IteratePointer->Text[i] == '>' ||
                            IteratePointer->Text[i] == '\0') {
                            EndLocation = i;  // End location found
                            break;
                        }
                    }
                    char *TempSubStr = getSubstring(IteratePointer->Text, StartLocation,
                                                    EndLocation - 1);  // Get file path

                    if (!StringEndsWith(TempSubStr,
                                        ".js"))  // Make sure .js is at the end of the file path
                    {
            ...
            else  // Script is inline JS so use a hacky way to make the inline JS it's own temporary
                  // file and link it to the HTML
            {
                ...
                for (int i = IteratePointer->StartIndex; i < textLength;
                     i++)  // Find the start location of the inline Javascript
                {
                ...
                if (StartLocation != -1)  // Start location has been found, search for end of inline JS
                {
                    ...
                    for (int i = StartLocation; i < strlen(TempFileContents);
                         i++)  // Search for end character (could cause problems when in comments)
                    {
                        if (TempFileContents[i] == '<') {
                            EndLocation = i;
                            break;
                    ...
                    {
                        char *NewJSContents = getSubstring(TempFileContents, StartLocation + 1,
                                                           EndLocation - 2);  // Get contents of the inline JS
                        ...
                        CreateFileWrite(filename,
                                        TempFileContents);  // Save new temporary HTML file

                        // Shift all dependencies used by the HTML by the amount the file has
                        // changed
                        ...
                        // CreateDependencyEdges((*DependencyGraph)->Vertexes[(*DependencyGraph)->VerticesNum
                        // - 1], DependencyGraph);
...
struct RegexMatch EMSCRIPTEN_KEEPALIVE *FindCSSDependencies(char *filename)
{
    char *FileContents = ReadDataFromFile(filename);
    struct RegexMatch *CommentLocations =
        GetAllRegexMatches(FileContents, "/\\*.*\\*/", 0, 0);  // Finds locations of comments
    struct RegexMatch *Dependencies = BasicRegexDependencies(filename, "@import [^;]*", 0, 0,
                                                             CommentLocations);  // Finds dependencies
    ...
    while (IteratePointer->IsArrayEnd != true)  // Loop through all matches
    {
        ...
        {
            if (IteratePointer->Text[i] == 'u' && IteratePointer->Text[i + 1] == 'r' &&
                IteratePointer->Text[i + 2] == 'l' && IteratePointer->Text[i + 3] == '(') {
                StartLocation = i + 5;
...
char *NodeModuleEntryPoint(char *NodeModulePath)
{
    ...
    if (FileExists(PackageJSONPath))  // Check if package.json exists
    {
        ...
        cJSON *main_field = cJSON_GetObjectItemCaseSensitive(
            PackageJSON, "main");  // Use cJSON to find the entry point specified in package.json
...
struct RegexMatch EMSCRIPTEN_KEEPALIVE *FindJSDependencies(char *filename)
{
    ...
    while (IteratePointer->IsArrayEnd != true)  // Loop through CommonJS dependencies
    {
        ...
        if (startLocation != -1 && endLocation != -1)  // Dependency name has been found
        {
            ...
            if (!StringEndsWith(IteratePointer->Text, ".cjs") &&
            ...
            else if (FileExists(TurnToFullRelativePath(IteratePointer->Text,
                                                       GetBasePath(filename))))  // Dependency name includes file
                                                                                 // extension check if it exists in CWD
            {

                LocalFileFound = true;
                IteratePointer->Text = TurnToFullRelativePath(IteratePointer->Text, GetBasePath(filename));
            }
            if (!LocalFileFound)  // Local module not found, it may be in node_modules or be a
    ...
    struct RegexMatch *ESDependencies =
        BasicRegexDependencies(filename, "import [^;]*from[^;]*;", 8, 0, CommentLocations);
    ...
    while (IteratePointer->IsArrayEnd != true)  // Loop over ES dependencies
    {
        ...
        for (int i = 0; i < strlen(IteratePointer->Text) - 6; i++) {
            if (strncasecmp(IteratePointer->Text + i, " from ", 6) == 0) {
                FromLocation = i + 5;  // Doesn't break so that function won't be messed up if
                                       // function name includes " from "
            }
        }
        if (FromLocation == -1) {
            ...
            if (StartLocation != -1)  // Check start location has been found
            {
                ...
                {
                    IteratePointer->Text = TurnToFullRelativePath(
                        getSubstring(IteratePointer->Text, StartLocation, EndLocation), GetBasePath(filename));

                    if (!StringEndsWith(IteratePointer->Text,
                                        ".js"))  // Make sure file name ends with .js
                    {
    ...
    CombineRegexMatchArrays(&CJSDependencies,
                            &ESDependencies);  // Combine the commonJS and ES dependency arrays together
