                                        ...
                                        int pluginIndex)
{
    ...
    for (int i = 0; i < DependencyGraph->VerticesNum; i++)  // Loop through each dependency in the graph
    {
        ...
        while (CJSONElement)  // Run every plugin in array
        {
            ...
            if (MatchGlob(DependencyGraph->Vertexes[i]->path,
                          CJSONElement->string))  // Check if the plugin glob matches the file
            {
                CJSONElement2 = CJSONElement->child;
                char *NewFileContents = NULL;
                while (CJSONElement2) {
                    char *Result;
                    char *ExtensionPath = malloc(strlen(CJSONElement2->valuestring) +
                                                 strlen(GetSetting("INTERNAL_FULL_CONFIG_PATH")->valuestring) + 2);
                    strcpy(ExtensionPath, GetSetting("INTERNAL_FULL_CONFIG_PATH")->valuestring);
                    strcat(ExtensionPath, CJSONElement2->valuestring + 2 * (CJSONElement2->valuestring[0] == '.' &&
                                                                            CJSONElement2->valuestring[1] == '/'));
                    ...
                    {
                        Result =
                            (char *)(*functionPTR)(ReadDataFromFile(DependencyGraph->Vertexes[i]->path), ExtensionPath,
                    ...
                    {
                        if ((*functionPTR)(ReadDataFromFile(DependencyGraph->Vertexes[i]->path), ExtensionPath,
                                           DependencyGraph->Vertexes[i]->path) != NULL) {
