...
int EMSCRIPTEN_KEEPALIVE GetNumOfRegexMatches(char *Text, const char *Pattern)
{
    ...
    {
        ThrowFatalError("Could not compile regex pattern: %s",
                        Pattern);  // Exits if an error occured during regex compilation
    };
    while (1) {
...
struct RegexMatch EMSCRIPTEN_KEEPALIVE *GetRegexMatch(char *Text, char *Pattern)
{
    ...
    {
        ThrowFatalError("Could not compile regex pattern: %s",
                        Pattern);  // Throws a fatal error if an error occured during regex compilation
    };
...
struct RegexMatch EMSCRIPTEN_KEEPALIVE *GetAllRegexMatches(char *Text, const char *Pattern, unsigned int StartPos,
                                                           unsigned int EndPos)
{
    const int N_MATCHES = 512;
    ...
    while (1) {
        int error = regexec(&regexp, TextStartPointer, N_MATCHES, match, 0);
        if (error == 0) {
            matches = (struct RegexMatch *)realloc(matches,
                                                   (matchesCompleted + 2) *
                                                       RegexMatchesSize);  // Reallocates memory for matches array

            matches[matchesCompleted].Text =
                getSubstring(TextStartPointer, (int)match[0].rm_so + StartPos,
                             (int)match[0].rm_eo - EndPos);  // Adds substring to matchs array
            matches[matchesCompleted].IsArrayEnd = false;
            matches[matchesCompleted].StartIndex =
                (unsigned int)match[0].rm_so +
                AmountShifted;  // Saves start index so it doesn't need to be recalculated later
            matches[matchesCompleted].EndIndex =
                (unsigned int)match[0].rm_eo + AmountShifted;  // Does the same as above but for the end index
            TextStartPointer += (int)match[0].rm_eo;  // Uses pointer arithmetic to set textStartPointer to end of match
