...
RebuildFiles(struct Graph *DependencyGraph, char *EncodedFiles,
             int FilesNum)  // Currently only used for arrowpack dev server to rebuild files that are changed
{
    printf("Rebuilding...\n");
    ...
    for (int i = 0; i < DependencyGraph->VerticesNum; i++)  // Finds the vertices for each file that has been modified
    {
        for (int j = 0; j < FilesNum; j++)  // Loops through each file that has changed to find it in the graph
        {
            if (strcasecmp(DependencyGraph->Vertexes[i]->path, Files[j]) == 0 ||
                strcasecmp(EntryToPreprocessPath(Files[j]), DependencyGraph->Vertexes[i]->path) ==
                    0)  // Checks if the search vertex is the file that has changed
            {
                FileFound = true;

                int TempVertexNum = 0;
                struct Node **TempVertexArray =
                    FindAllDependentsOfVertex(DependencyGraph->Vertexes[i], DependencyGraph->VerticesNum,
                                              &TempVertexNum);  // Gets all of the dependents of the changed file
                ...
                if (StringStartsWith(DependencyGraph->Vertexes[i]->path, PREPROCESS_DIR)) {
                    DependencyGraph->Vertexes[i]->path = ReplaceSectionOfString(DependencyGraph->Vertexes[i]->path, 0,
                                                                                30, GetSetting("entry")->valuestring);
                }

                CreateDependencyEdges(DependencyGraph->Vertexes[i],
                                      &DependencyGraph);  // Research dependencies for the changed file

                struct Edge *NewTempEdge = DependencyGraph->Vertexes[i]->edge;
                while (NewTempEdge != NULL)  // Loop through dependencies of vertex
                {
                    if (StringStartsWith(NewTempEdge->vertex->path,
                                         PREPROCESS_DIR))  // Rebuild dependencies in the preprocess directory
                    {
    ...
    if (FileFound) {
        FinalChangedFiles =
            ArrowSerialize((const char **)ChangedFiles,
                           TempGraph->VerticesNum);  // Serialise array of all changed files into a string to send to JS
    }
    else  // File is a new created file
    {
        for (int i = 0; i < FilesNum; i++)  //
        {
            struct Node *NewVertex = create_vertex(Files[i], GetFileTypeID(Files[i]),
                                                   NULL);  // Create vertex for the new file
            ...
            add_vertex(DependencyGraph,
                       NewVertex);         // Add the vertex to the dependency graph (doesn't need to be
                                           // resorted because the vertex is at the end of the graph and
