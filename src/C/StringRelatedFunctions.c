...
char *GetFileExtension(const char *PATH)
{
    ...
    if (lastFullStop == -1) {
        return strdup(path);  // If no '.' character is found, return the path itself for file such
                              // as "LICENSE"
    }
...
char EMSCRIPTEN_KEEPALIVE *TurnToFullRelativePath(const char *PATH, char *BasePath)
{
    ...
    {
        NewPath = malloc(sizeof(char) * (strlen(path) + strlen(GetSetting("entry")->valuestring)) +
                         1);                                // Allocate new entry path string
        ...
        if (MatchesNum > 0)                                      // Handles paths containing ../
        {
            ...
            char *NewPath = strdup(BasePath);  // Create a copy of the BasePath variable so it
                                               // doesn't get overwritten by strtok()
            ...
            for (int i = BasePathLength; i > 0; i--)  // Search backwards through to find the index of how
                                                      // many directories to cut off of the base path
            {
                if (NewPath[i] == '/') {
                    PathSeperatorsFound++;
        ...
        {
            if (strstr(path, GetSetting("entry")->valuestring) != NULL ||
                BasePath[0] == '\0')  // path is already full path (might accidentally include paths
                                      // with entry name in folder path)
            {
                return path;
            }
            if (StringStartsWith(path, PREPROCESS_DIR))  // Path is in preprocess temp directory
            {
                ...
                NewPath = malloc((strlen(BasePath) + strlen(path) + 1) *
                                 sizeof(char));  // Allocate new path with length of basepath, path
                ...
                strcat(NewPath,
                       TempPath2 + 2 * (TempPath2[0] == '.' &&
                                        TempPath2[1] == '/'));  // Add path to new path and remove "./" if present
            }
            return NewPath;
        }
    }
...
bool IsPreprocessDir(const char *path)
{
    ...
    return strncmp(path, PREPROCESS_DIR, 29) ==
           0;  // Use strncmp to check for preprocess path at the start of the string
}

// Gives the true base path without adjusting for preprocess directory, mainly useful for file
// operations
char *EMSCRIPTEN_KEEPALIVE GetTrueBasePath(const char *filename)
{
    ...
    memcpy(BasePath, filename,
           LastPathChar);  // Copy filepath up to the last past seperator to get the base path
...
char *InsertStringAtPosition(char *OriginalString, char *ReplaceString, int position)
{
    ...
    char *NewString = malloc(OriginalLen + ReplaceLen + 1);  // Allocate a new string with the length of the
                                                             // origininal string, new string a null terminator
    strncpy(NewString, OriginalString,
            position);  // Copy the start of the original string, up to the insert index, into the
                        // new string
    strcpy(NewString + position,
           ReplaceString);  // Copy the insert string into the new string at the insert index
    strcpy(NewString + position + ReplaceLen,
           OriginalString + position);  // Copy the rest of the original string into the new string
    return NewString;
}
...
char *EntryToExitPath(const char *path)
{
    ...
    if (strncasecmp(PathCopy, "node_modules/", 13) == 0) {
        return ReplaceSectionOfString(
            PathCopy, 0, 13,
            GetSetting("exit")->valuestring);  // Returns the exit path if the entry path if in node_modules
    }
    if (strncasecmp(PathCopy, PREPROCESS_DIR, 29) == 0) {
        return ReplaceSectionOfString(PathCopy, 0, 30,
                                      GetSetting("exit")->valuestring);  // Returns the exit path if the entry path is
                                                                         // in the preprocess directory
    }
    else {
        return ReplaceSectionOfString(
            PathCopy, 0, strlen(GetSetting("entry")->valuestring),
            GetSetting("exit")->valuestring);  // Replaces the entry path in the string with the exit path
    }
}
...
{
    return ReplaceSectionOfString(path, 0, strlen(GetSetting("entry")->valuestring) - 1,
                                  PREPROCESS_DIR);  // Replaces entry string with preprocess
}
...
bool StringEndsWith(char *str, char *substr)
{
    ...
    if (substr_len > str_len)  // If the substring length is more than the string length then the
                               // string can't end with it
    {
        return false;
    }
...
bool IsURL(char *str)
{
    ...
    return HasRegexMatch(str, "((http|https):\\\\/\\\\/)?[\\\\w\\\\-_]+(\\\\.[\\\\w\\\\-_]+)+([\\\\w\\\\-\\\\.,@?^=%&:/"
                              "~\\\\+#]*[\\\\w\\\\-\\\\@?^=%&/~\\\\+#])?");
}
...
ArrowDeserialize is a function that deserializes a string that has been serialized using the
ArrowSerialize function from Javascript

@param serialized - the serialized string

@param n_strings - a pointer to an integer that will be set to the number of strings found in the
serialized string
...
// Internal function used by match glob to expand braces in a glob pattern into a string with |
// delimeters
