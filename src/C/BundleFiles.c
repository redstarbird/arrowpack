    ...
    if (GetSetting("bundleCSSInHTML")->valueint == true)  // CSS will only be bundled into HTML if the setting is active
    {
        ...
        if (StyleResults[0].IsArrayEnd)                           // Style tag doesn't already exist
        {
            ...
            if (HeadTagResults[0].IsArrayEnd == false)                    // If <head> tag is found
            {
                ...
                ColorYellow();
                printf("No <head> tag found for file: %s, unable to bundle CSS into file, file "
                       "will still work\n",
                       GraphNode->path);
                ColorReset();
            }
...
void BundleJSinJS()
{
    ...
    struct ImportedESM *ImportedFunctionNames =
        malloc(sizeof(struct ImportedESM));  // Used to keep track of imported functions tht need to
                                             // have a unique name
    ...
    struct RegexMatch *FunctionNames =
        GetAllRegexMatches(InsertText, "function [^(]*", 9, 1);                             // Find all function names
    ...
    if (ISESModule)  // If the current dependency is an ESModule then this code is run to find the
                     // imports
    {

        if (HasRegexMatch(ReferenceText,
                          "import\\s+\\*"))  // Detects if the import is importing all imports
        {
            ImportAll = true;
            if (HasRegexMatch(ReferenceText,
                              "import\\s+\\*\\s+as"))  // Checks if the import is using an alias
            {
                ImportAllAlias = true;
            }
        }
            ...
            {
                int EndLocation =
                    LastOccurenceOfChar(ReferenceText, '}');  // Gets the end location of the named exports
                ...
                for (int i = LastOccurenceOfChar(ReferenceText, '{'); i <= EndLocation;
                     i++)  // Loops through named imports to find the names and their aliases
                {
                    ...
                    else if (InAlias)  // Checks for end of alias if currently searching for end
                    {
                        ...
                        {
                            ImportedFunctionNames[ImportedFunctionNameLength - 1].alias =
                                getSubstring(ReferenceText, CurrentVariableStart,
                                             i - 1);  // Set the ImportedESM alias to the alias found
                    ...
                    else if (!InVariable)  // Checks if it has reached to start of a variable
                    {
                        if (!IsEndOfJSName(ReferenceText[i]))  // A new variable has been found
                        {
                            ...
                            ImportedFunctionNames = realloc(
                                ImportedFunctionNames,
                                sizeof(struct ImportedESM) * (ImportedFunctionNameLength +
                                                              1));  // Resive imported function struct to have space for
                            ImportedFunctionNames[ImportedFunctionNameLength].IsArrayEnd =
                                true;  // Used for end of array when iterating
                        }
                    }
                    else {
                        if (IsEndOfJSName(ReferenceText[i])) {
                            ...
                            {
                                if (ReferenceText[CurrentVariableStart] == 'a' &&
                                    ReferenceText[CurrentVariableStart + 1] ==
                                        's')  // detects if alias is used for previous variable
                                {
                                    AfterAs = true;
                                    InVariable = false;
                                    ImportedFunctionNameLength--;  // Deincrement because "as" it
                                                                   // not an imported function/value
                                }
                            }
                            if (!AfterAs)  // Previous variable is an actual variable and not the
                                           // "as" keyword
                            {
                                struct ImportedESM *FunctionName =
                                    &ImportedFunctionNames[ImportedFunctionNameLength - 1];
                                FunctionName->name = getSubstring(ReferenceText, CurrentVariableStart,
                                                                  i - 1);  // Set function name
                                FunctionName->alias = NULL;  // Set alias to NULL as it hasn't been found yet
                                ImportedFunctionNames[ImportedFunctionNameLength - 1].IsDefault =
                                    false;  // Sets variable as not being a default imported
                                            // function
                ...
                ImportDefault =
                    HasRegexMatch(ReferenceText,
                                  "import[^,;]*,[^;]*{[^;]*}[^;]*;");  // Uses regex to find if a default export
                                                                       // is imported aswell as named imports
            }
            else {
                ImportDefault = true;
            }
            ...
            if (ImportDefault)  // If a default import is present then find it's name and alias
            {
                ...
                {
                    DefaultExport =
                        GetRegexMatch(InsertText, "export\\s+default");  // Find location of the default export
                ...
                bool AfterAs = false;
                int CheckEnd =
                    LastOccurenceOfChar(ReferenceText, '{');  // Finds the end of where to check the default import
                if (CheckEnd == -1) {
                ...
                for (int i = 7; i < CheckEnd; i++)  // Get default import and find name and alias
                {
                    if (NameFound)  // Check if the name is found
                    {
                        if (AfterAs) {
                            if (StartLocation == -1) {
                                if (!IsEndOfJSName(ReferenceText[i]))  // Finds the start of the name
                                                                       // of the default import alias
                                {
                                    StartLocation = i;
                                }
                            }
                            else {
                                if (IsEndOfJSName(ReferenceText[i]))  // Finds the end of the name
                                                                      // of the default import alias
                                {
                                    ImportedFunctionNames[ImportedFunctionNameLength - 1].alias =
                                        getSubstring(ReferenceText, StartLocation, i - 3);
                                }
                            }
                        }
                    }
                    ...
                    else if (StartLocation != -1)  // Start location of default name/alias has not yet been found
                    {
                        if (IsEndOfJSName(ReferenceText[i])) {
                            ImportedFunctionNameLength++;
                            ImportedFunctionNames = realloc(
                                ImportedFunctionNames, sizeof(struct ImportedESM) * (ImportedFunctionNameLength + 1));
                            ...
                            FunctionName.name =
                                getSubstring(InsertText, DefaultExport->EndIndex + 10, DefaultExportNameEnd);
        ...
        {
            ESMIteratePointer = &ImportedFunctionNames[0];  // Loops through functions that are being imported to
                                                            // remove uneeded exported functions
            if (!ImportAll)  // Removes all exported functions that are not being imported so they
                             // can have name collisions fixed
            {
            ...
            while (IteratePointer2->IsArrayEnd != true) {
                if (IteratePointer2->StartIndex > IteratePointer->StartIndex &&
                    IteratePointer2->StartIndex < IteratePointer->EndIndex) {
            ...
            if (HasRegexMatch(IteratePointer->Text, "export\\s+default")) {
                RemoveRegexMatch(IteratePointer);  // Doesn't break in the case of the JS being
                                                   // invalid and having 2 default exports
            }

            IteratePointer++;
        }
        IteratePointer = &FunctionNames[0];  // Set iteration pointer to the first function name to
                                             // loop over later
    }
    else  // If the current dependency is a CommonJS module then this code is run to find the
          // exports
    {
        ...
        if (strlen(IteratePointer->Text) > 1)  // Ignores unamed functions
        {
            ...
            if (FunctionDuplicateFound)  // File contains at least one name collision
            {
                ...
                for (int i = 0; i < LoopLength; i++)  // Loop through the file
                {
                    ...
                    else if (strncmp(InsertText + i, IteratePointer->Text,
                                     strlen(IteratePointer->Text)) == 0 &&
                             !InString)  // Replace name collision with replacement name
                    {
                        InsertText =
                            InsertStringAtPosition(InsertText, NewUnusedName, i + strlen(IteratePointer->Text));
                        int InverseShiftAmount =
                            GetInverseShiftedAmount(i + strlen(IteratePointer->Text), JSFileShiftLocations);
                        AddShiftNum(InverseShiftAmount, strlen(NewUnusedName), &JSFileShiftLocations,
                                    &JSShiftLocationsLength);
                    }
                }
            }
        }
    ...
    if (ISESModule)  // Bundles the files together if they are ES modules
    {
        ...
        {
            char *NewDefinition;
            if (ESMIteratePointer->alias != NULL ||
                ESMIteratePointer->IsDefault)  // Check if the function/variable has an alias
            {
                if (ESMIteratePointer->IsDefault) {
                    NewDefinition =
                        malloc(sizeof(char) * strlen(ESMIteratePointer->alias) + strlen(ESMIteratePointer->name) + 7);
                }
                else {
                    NewDefinition = malloc(sizeof(char) *
                                           (10 + strlen(ESMIteratePointer->alias) + strlen(ESMIteratePointer->name)));
                }
                ...
                InsertText = realloc(InsertText, sizeof(char) * strlen(InsertText) + strlen(NewDefinition) +
                                                     1);         // Adds space for new alias definition
                InsertText = strcat(InsertText, NewDefinition);  // Adds alias definition to the end of the
    ...
    else  // Bundles the files together if they are CommonJS modules
    {
        ...
        while (IteratePointer != UsableExtraImports && IteratePointer->IsArrayEnd == false) {
            ...
            AddShiftNum(IteratePointer->StartIndex, (IteratePointer->EndIndex - IteratePointer->StartIndex) * -1,
                        &JSFileShiftLocations, &JSShiftLocationsLength);
            IteratePointer++;
        }
        ...
        {
            InsertText = InsertStringAtPosition(InsertText, ModuleObjectDefinition,
                                                GetShiftedAmount(FinalElement->StartIndex, JSFileShiftLocations));
            AddShiftNum(FinalElement->StartIndex, strlen(ModuleObjectDefinition), &JSFileShiftLocations,
                        &JSShiftLocationsLength);
            InsertText = ReplaceSectionOfString(
                InsertText, GetShiftedAmount(FinalElement->StartIndex, JSFileShiftLocations),
                GetShiftedAmount(FinalElement->StartIndex, JSFileShiftLocations) + 14, NewModuleExportsName);
            AddShiftNum(FinalElement->StartIndex, strlen(NewModuleExportsName) - 14, &JSFileShiftLocations,
                        &JSShiftLocationsLength);
        }
        else {
            InsertText = InsertStringAtPosition(InsertText, ModuleObjectDefinition, UsableExtraImports->StartIndex);
            AddShiftNum(UsableExtraImports->StartIndex, strlen(ModuleObjectDefinition), &JSFileShiftLocations,
                        &JSShiftLocationsLength);
        }

        while (!UsableExtraImports->IsArrayEnd) {
            InsertText = ReplaceSectionOfString(
                InsertText, GetShiftedAmount(UsableExtraImports->StartIndex, JSFileShiftLocations),
                GetShiftedAmount(UsableExtraImports->StartIndex + 8, JSFileShiftLocations), NewModuleExportsName);
            AddShiftNum(UsableExtraImports->StartIndex, strlen(NewModuleExportsName) - 8, &JSFileShiftLocations,
                        &JSShiftLocationsLength);
            UsableExtraImports++;
        }
    }

    if (GetSetting("productionMode")->valueint ==
        false)  // Keeps line numbers the same by turning new import into one line
    {
        ...
        if (!HeadTagResults[0].IsArrayEnd)                            // Check if head tag is present
        {
                ...
                GetSetting("faviconPath")->valuestring != NULL)  // Check if a favicon path is provided
            {
                ...
                if (FaviconTagResults == NULL)  // Check there in not already a favicon
                {
                    ...
                    if (StringStartsWith(GetSetting("faviconPath")->valuestring,
                                         GetSetting("entry")->valuestring))  // Check if favicon path
                                                                             // includes entry path
                    {
                        strcat(FaviconLink,
                               GetSetting("faviconPath")->valuestring +
                                   strlen(GetSetting("entry")->valuestring));  // Add favicon path without entry path
                    }
                    else {
                        strcat(FaviconLink,
                               GetSetting("faviconPath")->valuestring);  // Add the whole favicon path because it
...
void PostProcessFile(struct Node *node, struct Graph *graph)
{
    ...
    if (node->FileType == JSFILETYPE_ID)  // Postprocessing for JS files
    {
        ...
        while (IteratePointer->IsArrayEnd == false) {
            RemoveSectionOfString(FileContents, GetShiftedAmount(IteratePointer->StartIndex, shiftLocations),
                                  GetShiftedAmount(IteratePointer->EndIndex, shiftLocations) + 1);
            AddShiftNum(IteratePointer->StartIndex, IteratePointer->EndIndex - IteratePointer->StartIndex - 1,
                        &shiftLocations, &ShiftlocationLength);
            IteratePointer++;
        }
...
bool EMSCRIPTEN_KEEPALIVE BundleFiles(struct Graph *graph)
{
    ...
    ColorNormal();  // Resets colours incase of a colour not being previously reset and messing up
                    // the user's terminal
    IsNodeBuiltin(NULL);
